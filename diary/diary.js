(() => {
  ...
  const DELETE_CONFIRM_CODE = "0714";
  ...
  let imageSelectionChanged = false;
  let currentPage = 1;
  ...
  async function saveEntry(event) {
    ...
    try {
      ...
      const imagesToKeep = entryBeingEdited && !imageSelectionChanged
        ? entryBeingEdited.images
        : existingImages;
      ...
      const savedRow = {
        ...result.data,
        // 저장 직후 화면은 이번에 보낸 payload를 기준으로 표시합니다.
        images: nextImages,
        cover_image: payload.cover_image,
      };
      let savedEntry = parseSupabaseEntry(savedRow);
      ...
      if (entryBeingEdited) {
        ...
        const nextImagePaths = nextImages.map((image) => image.path);
  ...
  async function deleteEntry() {
    ...
    const deleteCode = window.prompt(
      `비밀번호를 입력해 주세요.\n“${entry.title}” 기록을 정말 삭제할까요?`,
      "",
    );
    if (deleteCode === null) return;
    if (deleteCode.trim() !== DELETE_CONFIRM_CODE) {
      window.alert("입력이 맞지 않아 삭제하지 않았어요.");
      return;
    }
  ...
  entryImagesInput.addEventListener("change", (event) => {
    ...
    if (acceptedFiles.length > 0) imageSelectionChanged = true;
    event.target.value = "";
    renderSelectedImages();
  });
